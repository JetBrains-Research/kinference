...
suspend fun softmaxZero(
    ...
    return when(input.type) {
        DataType.FLOAT -> softmaxZeroFloat(input as FloatNDArray, rows, columns)
        DataType.DOUBLE ->  softmaxZeroDouble(input as DoubleNDArray, rows, columns)
...
suspend fun softmaxZero(
    ...
    return when(input.type) {
        DataType.FLOAT -> softmaxZeroFloat(input as FloatNDArray, dest as MutableFloatNDArray, rows, columns)
        DataType.DOUBLE ->  softmaxZeroDouble(input as DoubleNDArray, dest as MutableDoubleNDArray, rows, columns)
        else -> error("Softmax operation supported only for FLOAT and DOUBLE tensors, actual type is ${input.type}")
    }
}
