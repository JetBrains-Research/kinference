...
import io.kinference.ndarray.extensions.reduce.l2.reduceL2
import io.kinference.operator.*
import io.kinference.protobuf.message.AttributeProto
import io.kinference.protobuf.message.TensorProto
import io.kinference.utils.toIntArray

sealed class ReduceL2(name: String, info: OperatorInfo, attributes: Map<String, Attribute<Any>>, inputs: List<String>, outputs: List<String>) :
    ...
    companion object {
        ...
        operator fun invoke(name: String, version: Int?, attributes: Map<String, Attribute<Any>>, inputs: List<String>, outputs: List<String>) =
            when (version ?: DEFAULT_VERSION.sinceVersion) {
                in ReduceL2Ver1.VERSION.asRange() -> ReduceL2Ver1(name, attributes, inputs, outputs)
                in ReduceL2Ver18.VERSION.asRange() -> ReduceL2Ver18(name, attributes, inputs, outputs)
                else -> error("Unsupported version of ReduceL2 operator: $version")
            }
    }
}
...
class ReduceL2Ver1(
...
) : ReduceL2(name, INFO, attributes, inputs, outputs) {
    companion object {
        ...
        private val INFO = OperatorInfo("ReduceL2", ATTRIBUTES_INFO, INPUTS_INFO, OUTPUTS_INFO, VERSION, OperatorInfo.DEFAULT_DOMAIN)
    }
    ...
    override suspend fun <D : ONNXData<*, *>> apply(contexts: Contexts<D>, inputs: List<KITensor?>): List<KITensor?> {
        ...
        return listOf(input.reduceL2(actualAxes, keepDims).asTensor("reduced"))
    }
}
