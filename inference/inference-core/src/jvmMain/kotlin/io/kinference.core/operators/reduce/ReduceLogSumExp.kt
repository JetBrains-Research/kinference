...
import io.kinference.ndarray.extensions.reduce.logSumExp.reduceLogSumExp
import io.kinference.operator.*
import io.kinference.protobuf.message.AttributeProto
import io.kinference.protobuf.message.TensorProto
import io.kinference.utils.toIntArray

sealed class ReduceLogSumExp(name: String, info: OperatorInfo, attributes: Map<String, Attribute<Any>>, inputs: List<String>, outputs: List<String>) :
    ...
    companion object {
        ...
        operator fun invoke(name: String, version: Int?, attributes: Map<String, Attribute<Any>>, inputs: List<String>, outputs: List<String>) =
            when (version ?: DEFAULT_VERSION.sinceVersion) {
                in ReduceLogSumExpVer1.VERSION.asRange() -> ReduceLogSumExpVer1(name, attributes, inputs, outputs)
                in ReduceLogSumExpVer18.VERSION.asRange() -> ReduceLogSumExpVer18(name, attributes, inputs, outputs)
                else -> error("Unsupported version of ReduceLogSumExp operator: $version")
            }
    }
}


class ReduceLogSumExpVer1(
    name: String,
    attributes: Map<String, Attribute<Any>>,
    inputs: List<String>, outputs: List<String>
) : ReduceLogSumExp(name, INFO, attributes, inputs, outputs) {
    companion object {
        ...
        private val INFO = OperatorInfo("ReduceLogSumExp", ATTRIBUTES_INFO, INPUTS_INFO, OUTPUTS_INFO, VERSION, OperatorInfo.DEFAULT_DOMAIN)
    }
    ...
    override suspend fun <D : ONNXData<*, *>> apply(contexts: Contexts<D>, inputs: List<KITensor?>): List<KITensor?> {
        ...
        return listOf(input.reduceLogSumExp(actualAxes, keepDims).asTensor("reduced"))
    }
}

class ReduceLogSumExpVer18(
    name: String,
    attributes: Map<String, Attribute<Any>>,
    inputs: List<String>, outputs: List<String>
) : ReduceLogSumExp(name, INFO, attributes, inputs, outputs) {
