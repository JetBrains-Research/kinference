...
class SequenceEraseVer11 internal constructor(
    ...
    override suspend fun <D : ONNXData<*, *>> apply(contexts: Contexts<D>, inputs: List<KIONNXData<*>?>): List<KIONNXSequence> {
        val seq = inputs[0]!! as KIONNXSequence
        val seqList = ArrayList(seq.data)
        val positionTensor = inputs.getOrNull(1)?.data as? NumberNDArrayCore

        val position = (positionTensor?.singleValue() as? Number)?.toInt() ?: (seqList.size - 1)
