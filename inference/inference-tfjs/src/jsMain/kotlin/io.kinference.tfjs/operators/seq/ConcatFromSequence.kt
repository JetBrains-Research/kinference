...
sealed class ConcatFromSequence(
    name: String,
    info: OperatorInfo,
...
class ConcatFromSequenceVer11(name: String, attributes: Map<String, Attribute<Any>>, inputs: List<String>, outputs: List<String>) : ConcatFromSequence(name, INFO, attributes, inputs, outputs) {
    ...
    override suspend fun <D : ONNXData<*, *>> apply(contexts: Contexts<D>, inputs: List<TFJSSequence?>): List<TFJSTensor?> {
        val srcTensors = (inputs.first()!!.data as List<TFJSTensor>).map { it.data }
        val tensor = if (newAxis) srcTensors.stack(axis) else srcTensors.concat(axis)
        return listOf(tensor.asTensor("concat_result"))
    }
}
