...
class TestDynamicQuantizeLSTM {
    ...
    fun test_LSTM_defaults()  = TestRunner.runTest {
        AccuracyRunner.runFromResources(getTargetPath("test_lstm_defaults"), delta = 1e-2)
    }

    @Test
    fun test_LSTM_with_initial_bias()  = TestRunner.runTest {
        AccuracyRunner.runFromResources(getTargetPath("test_lstm_with_initial_bias"), delta = 1e-2)
    }

    @Test
    fun test_LSTM_with_peepholes() = TestRunner.runTest {
        AccuracyRunner.runFromResources(getTargetPath("test_lstm_with_peepholes"), delta = 1e-2)
    }

    @Test
    fun test_BiLSTM_defaults()  = TestRunner.runTest {
        AccuracyRunner.runFromResources(getTargetPath("test_bilstm_defaults"), delta = 1e-2)
    }
}
