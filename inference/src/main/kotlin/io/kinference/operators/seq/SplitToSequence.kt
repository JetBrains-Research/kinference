...
import io.kinference.types.ValueInfo.SequenceInfo
...
class SplitToSequence(attributes: Map<String, Attribute<Any>>, inputs: List<String>, outputs: List<String>)
    : Operator<Tensor, Sequence>(INFO, attributes, inputs, outputs) {
    companion object {
    ...
    override fun apply(context: Context, inputs: List<Tensor?>): List<Sequence?> {
        ...
        return listOf(Sequence(tensors, SequenceInfo("output_sequence", SequenceTypeInfo(tensors[0].info.type))))
    }
}
